...
class styleGANLinear(nn.Module):
    '''
    Linear layer with equlized learning rate and custom learning rate multiplier.
    TODO: what is w_scale.
    TODO: multiplication with decimal value meaning..?
    '''
    ...
    def __init__(self,
        ...
        # initialized standard deviation
...
class TransposedConvLayer(nn.Module):
    def __init__(self, 
        ...
        bias = False if norm == 'BN' else True
        # upsampling by transpose convolution (learnable upsampling)
...
class UpsampleConvLayer(nn.Module):
    def __init__(self, 
        ...
        if activation is not None:
            self.activation = getattr(torch, activation, 'relu') # torch.nn.ReLU()
        else:
            self.activation = None
    ...
    def forward(self, x):
        # upsampling - increase height, width dimension by 2
...
class RecurrentConvLayer(nn.Module):
    ...
    def forward(self, x, prev_state):
        x = self.conv(x)
        state = self.recurrent_block(x, prev_state) # tuple(hidden state,cell state)
...
class ConvLSTM(nn.Module):
    # inputs, cell outputs, hidden states, gates are 3D tensor
    # # of row, column of input == # of row, column of hidden state
    ...
    def forward(self, input_, prev_state=None):
        ...
        if prev_state is None:
            ...
            if state_size not in self.zero_tensors:
                ...
                self.zero_tensors[state_size] = (
                    torch.zeros(state_size).to(input_.device), # hidden state
                    torch.zeros(state_size).to(input_.device) # cell
                )
        ...
        stacked_inputs = torch.cat((input_, prev_hidden), 1) # channel 방향으로 합치기
