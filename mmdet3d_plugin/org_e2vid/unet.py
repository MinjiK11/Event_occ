...
from org_e2vid.submodel import ConvLayer, UpsampleConvLayer, TransposedConvLayer, RecurrentConvLayer, ResidualBlock, ConvLSTM, ConvGRU
...
class UNetRecurrent(BaseUNet):
    ...
    def forward(self, x, prev_states): ## x : [1,5,256,256] ## x : [1,5,260,346]
        ...
        for resblock in self.resblocks:
            ...
            # cv2.imwrite(os.path.join(savepath,'res'+str(r)+'.png'),tmp[:,:,0])

        # decoder
        for i, decoder in enumerate(self.decoders):
            x = decoder(self.apply_skip_connection(x, blocks[self.num_encoders - i - 1]))
            #save_output(x,savepath,i)
...
# cv2.imwrite('/share/E2VID_share/share_dataset/outdoor_day2/test3.png',tmp[:,:,0])