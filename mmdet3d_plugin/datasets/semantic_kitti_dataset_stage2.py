...
class SemanticKittiDatasetStage2(Dataset):
    ...
    def get_data_info(self, index):
        ...
        event_id=frame_id[index:]

        if frame_id=='000000':
            event_id='1'

        if frame_id[index:]==str(len(self.poses[sequence])-1):
            event_id=str((len(self.poses[sequence]))-2)

        meta_dict = self.get_meta_info(scan, sequence, frame_id, proposal_path, event_id)
        img = self.get_input_info(sequence, frame_id, event_id)
    ...
    def get_meta_info(self, scan, sequence, frame_id, proposal_path, event_id):
    ...
    def get_input_info(self, sequence, frame_id, event_id):
